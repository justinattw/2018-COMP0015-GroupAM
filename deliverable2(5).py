...
def enter_votes():
    ...
    def pick_project():
        ...
        def check_if_proceed():
            ...
            """
            CONFIRMATION_OPTIONS = ('y', 'n')
            
            check = projects_dict[pick_project].members[1] 
            if check.votes != {}: #checks if there are already votes
                print('Project {} has already been voted on. Do you want to edit
                      'the votes?')
                
                confirmation = ""
                while confirmation not in CONFIRMATION_OPTIONS:
                      message = ("Enter 'y' to proceed and overwrite all votes "
                                 "on this project or 'n' to return to the main "
                                 "menu: ")
